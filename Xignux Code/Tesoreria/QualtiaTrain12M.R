...
year_start <- 2016
month_start <- 01
prediction_date_error <- '2020-09-01'
...
year_month <- seq(as.Date(prediction_date_error), by = "month", length.out = 12) %>% 
  data.frame() %>% 
  mutate(Date = as.numeric(format(as.Date(.), '%Y%m%d')),
         Month = substr(Date, 5, 6),
         Year = substr(Date, 1, 4)) %>% 
...
data_model <- data[1:(nrow(data) - 11), 1]

data_evaluation <- data[(nrow(data)-11):nrow(data), 1]
