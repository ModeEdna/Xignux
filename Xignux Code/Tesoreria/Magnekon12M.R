...
year_start <- 2017
month_start <- 1
prediction_date_error <- '2020-08-01'
length_prediction <- 12
...
data <- read_csv(paste0("C:/Users/Eduardo.Armenta/Desktop/Xignux/Proyectos/Tesoreria/Data/Datos", business, ".csv")) %>% 
  select(Amount) %>% 
...
# Note that we need to change prediction_date_error for 6 or 12 months

# Create again year month depending on the number of months to be evaluated

year_month <- seq(as.Date(prediction_date_error), by = "month", length.out = length_prediction) %>% 
  data.frame() %>% 
  mutate(Date = as.numeric(format(as.Date(.), '%Y%m%d')),
         Month = substr(Date, 5, 6),
         Year = substr(Date, 1, 4)) %>% 
...
data_model <- data[1:(nrow(data) - length_prediction), 1]

data_evaluation <- data[(nrow(data) - (length_prediction - 1)):nrow(data), 1]
...
for (p in 1:3) for (q in 1:2) for (P in 0:2) {
  ...
  prediction <- sarima.for(data_model_diff, n.ahead = 12, p = p, d = 1, q = q, P = P, D = 0, Q = 1, S = 12)
...
data <- read_csv(paste0("C:/Users/Eduardo.Armenta/Desktop/Xignux/Proyectos/Tesoreria/Data/Datos", business, ".csv")) %>% 
  ...
  left_join(prediction_df %>% 
              filter(p == pred_results$p[1],
                     q == pred_results$q[1],
                     P == pred_results$P[1]) %>% 
              select(Month, Year, Prediction, Standard_Error, Standard_Error_2, Standard_Error_N, Standard_Error_2_N),
...
data_graph %>% 
  ...
  scale_x_discrete(breaks = data_graph$Year_Month[seq(1, nrow(data_graph), 3)]) +
